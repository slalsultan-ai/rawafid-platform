...
import { Loader2, ChevronUp, ChevronDown, FlaskConical } from "lucide-react";
...
const TEST_USERS = [
  {
    ...
    color: "bg-violet-500",
    users: [
      { name: "مسؤول الهيئة", email: "admin@goid.gov.sa" },
    ],
  },
  {
    role: "mentor",
...
export function UserSwitcher() {
  ...
  return (
    <div className="fixed bottom-5 left-5 z-50 font-arabic" dir="rtl">
      ...
      {open && (
        <div className="mb-3 w-80 bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden">
          ...
          <div className="bg-gradient-to-l from-teal-600 to-emerald-600 px-4 py-3 flex items-center gap-2">
            ...
            <span className="text-white/60 text-xs mr-auto">كلمة المرور: Rawafid@2024</span>
          </div>
                ...
                <div className="space-y-1">
                  {group.users.map((u) => (
                    ...
                    >
                      ...
                      {/* Arrow */}
                      <span className="text-slate-300 group-hover:text-teal-500 transition-colors text-xs">
                        ←
                      </span>
                    </button>
                  ))}
      ...
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-2 bg-slate-900 hover:bg-slate-800 text-white px-4 py-2.5 rounded-xl shadow-xl transition-all hover:scale-105 active:scale-95"
      >
        <FlaskConical className="w-4 h-4 text-teal-400" />
        <span className="text-sm font-semibold">تبديل المستخدم</span>
        {open
