    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">{allUsers.length}</p>
        </div>
      </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-100 text-xs text-slate-500 uppercase tracking-wider">
                  <th className="text-start p-4">{t("userTable.user")}</th>
                  <th className="text-start p-4">{t("userTable.department")}</th>
                  <th className="text-start p-4">{t("userTable.jobTitle")}</th>
                  <th className="text-start p-4">{t("userTable.role")}</th>
                  <th className="text-start p-4">{t("userTable.status")}</th>
                </tr>
              </thead>
              <tbody>
                {allUsers.map((u) => (
                  ...
                  >
                    ...
                    <td className="p-4">
                      <Badge variant={roleColors[u.role] ?? "secondary"}>
                        {t(`roles.${u.role}` as `roles.${"super_admin" | "org_admin" | "mentor" | "mentee" | "employee"}`)}
                      </Badge>
                    </td>
                    <td className="p-4">
