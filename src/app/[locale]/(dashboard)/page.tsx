  ...
  const stats = db
    ? await Promise.all([
        ...
        db
          ...
          .from(matches)
          .where(
            and(eq(matches.tenantId, user.tenantId), eq(matches.status, "active"))
          )
          .then((r) => Number(r[0]?.count ?? 0)),
        db
          .select({ count: count() })
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          ...
          <Card>
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                ...
                {incomingRequests.length > 0 && <Badge variant="warning">{incomingRequests.length}</Badge>}
              </div>
            </CardHeader>
            <CardContent>
              {incomingRequests.length === 0 ? (
                <p className="text-sm text-slate-500">{t("noNewRequests")}</p>
              ) : (
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            ...
            >
              <span className="text-sm text-slate-700">{t("browseMentors")}</span>
              <ArrowRight className={`w-4 h-4 text-slate-400 group-hover:text-teal-600 ${isRTL ? "rotate-180" : ""}`} />
            </Link>
            <Link
              href={`/${locale}/mentors/register`}
              className="flex items-center justify-between py-2 hover:bg-slate-50 rounded-lg px-2 transition-colors group"
            >
              <span className="text-sm text-slate-700">{t("registerAsMentor")}</span>
              <ArrowRight className={`w-4 h-4 text-slate-400 group-hover:text-teal-600 ${isRTL ? "rotate-180" : ""}`} />
            </Link>
            {(user.role === "mentee" || user.role === "employee") && (
              <Link
                href={`/${locale}/mentoring/request`}
                className="flex items-center justify-between py-2 hover:bg-slate-50 rounded-lg px-2 transition-colors group"
              >
                <span className="text-sm text-slate-700">{t("requestMentoring")}</span>
                <ArrowRight
                  className={`w-4 h-4 text-slate-400 group-hover:text-teal-600 ${isRTL ? "rotate-180" : ""}`}
                />
              </Link>
            )}
          </CardContent>
